...
export type TtsAdapterOptions = {
  ...
  /**
   * When set, overrides the Spanish segment for S3 clip playback (`answer`,
   * `answer-medium`, or `answer-slow`). Used for pronunciation example, feedback
   * replay at slow speed, etc. Omitted uses `answer`. Ignored by on-the-fly TTS
   * adapters except for approximate playback-rate parity (see `packages/ai` useTTS).
   */
  spanishSegmentOverride?: 'answer' | 'answer-medium' | 'answer-slow';
  /**
