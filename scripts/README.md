...
| **`sync-transcripts/`** | **Push:** upsert into **`lesson_transcripts`** from default **`input/lessons/`** at repo root, or **`PUSH_TRANSCRIPTS_SOURCE_DIR`** / **`--source-dir`**, or one **`--file`**. **Pull:** **`npm run pull:transcripts`** → **`{base}/{id}.json`**. | [`sync-transcripts/README.md`](sync-transcripts/README.md) |
