...
| **`sync-transcripts/`** | **Push:** **`{ meta, phrases }`** lesson files → **`lesson_transcripts`** + **`lesson_catalog`**. Default **`input/lessons/`**, or **`PUSH_TRANSCRIPTS_SOURCE_DIR`** / **`--source-dir`**, or **`--file`**. **Pull:** same shape to **`{base}/{id}.json`**. | [`sync-transcripts/README.md`](sync-transcripts/README.md) |
